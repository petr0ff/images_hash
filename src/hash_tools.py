...
class Hashing:
    ...
    def build_dhash(self, image):
        ...
        start = time.time()
        image = self.convert_and_resize_image(image)
        # 3. Comparing pixel by pixel (with neighbor pixel)
        difference = self.collect_pixel_diff(image)
        # 4. Convert to hexadecimal array
        finish = time.time()
        self.current_duration = "%0.3f" % (finish - start)
        return self.convert_to_hex(difference)

    def convert_and_resize_image(self, image):
        # 'L' is for grayscale
    ...
    def collect_pixel_diff(self, image):
    ...
    def convert_to_hex(self, difference):
        # linear
        decimal_value = 0
        hex_string = []
